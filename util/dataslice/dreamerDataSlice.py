"""
模型训练所需的数据加载器文件。定义dreamer数据集的数据加载器。
"""
import re
import os
import csv
...
class DreamerDataSlice:
    """
    dreamer数据集的dataloader。
    """
    ...
    def load_data(file_path,seq_len):
        ...
        invalid = 0
        # 创建根目录
        os.makedirs(Path.DREAMER_SLICED_DATA_DIR)
        # 加载标签文件
        label = DreamerDataSlice.load_labels()
        # 加载P-EEG数据
        all_mat_file = os.walk(file_path)
        skip_set = {'DreamerLabel.xls'}
        for path, dir_list, file_list in all_mat_file:
            for file_name in file_list:
                if file_name not in skip_set:
                    ...
                    # 创建文件路径
                    dir_path = Path.DREAMER_SLICED_DATA_DIR + '/Subject_' + subject_id
                    # 开启数据库
                    ...
                    for i in range(18):
                        ...
                        # 获取当前试次的标签
                        arousal = label[(int(subject_id) - 1) * 18 + i][1]
                        valence = label[(int(subject_id) - 1) * 18 + i][2]
                        quality = label[(int(subject_id) - 1) * 18 + i][3]
                        ...
                        while start_at + seq_len < subject_eeg_trail.shape[1]:
                            ...
                            # 将切片数据信息写入表格
                            data = [start_at, end_at, clip_id, subject_id, i, valence, arousal, discrete_label, 0]
                            with open(dir_path + '/msg.csv', 'a', newline='') as file:
                                writer = csv.writer(file)
                                writer.writerow(data)
                            start_at = end_at
                        print("dreamer数据切片中...请等待...")
        print("dreamer各类样本量（正、负、无效）：")
        print(positive)
        print(negative)
        print(invalid)
